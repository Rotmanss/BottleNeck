...
class Expander(QtWidgets.QWidget):
    def __init__(self, boxes_func, area_func):
        super().__init__()
        self.messge = dialog_functions.DialogFunctions()
        self.append_combo_boxes = boxes_func
        self.parse_to_scroll_area = area_func
    ...
    def open_expander(self):
        ...
        def commit_button():
            ...
            and ranking_line.text() != '':
                self.result += f"-- ID: {id_line.text()} --\n"
                self.result += f"Surname: {surname_line.text()}\n"
                self.result += f"Faculty: {faculty_line.text()}\n"
                self.result += f"Major: {major_line.text()}\n"
                self.result += f"Department: {department_line.text()}\n"
                self.result += f"Evaluations: {evaluations_line.text()}\n"
                self.result += f"Ranking: {ranking_line.text()}"
                ...
                try:
                    self.parse_to_scroll_area(self.result)
                    self.append_combo_boxes(self.comboBoxStatusDict)
                except:
                    self.messge.wrong_data_input()
